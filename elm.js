...
function _Browser_visibilityInfo()
{
	return (typeof _VirtualDom_doc.hidden !== 'undefined')
		? { aA: 'hidden', av: 'visibilitychange' }
		:
	(typeof _VirtualDom_doc.mozHidden !== 'undefined')
		? { aA: 'mozHidden', av: 'mozvisibilitychange' }
		:
	(typeof _VirtualDom_doc.msHidden !== 'undefined')
		? { aA: 'msHidden', av: 'msvisibilitychange' }
		:
	(typeof _VirtualDom_doc.webkitHidden !== 'undefined')
		? { aA: 'webkitHidden', av: 'webkitvisibilitychange' }
		: { aA: 'hidden', av: 'visibilitychange' };
}
...
var _Http_toTask = F3(function(router, toTask, request)
{
	return _Scheduler_binding(function(callback)
	{
		...
		try {
			xhr.open(request.aE, request.al, true);
		} catch (e) {
...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {P: fragment, S: host, W: path, Y: port_, aa: protocol, ab: query};
	});
...
var $author$project$Common$Breed = F3(
	function (name, subBreeds, imageUrls) {
		return {aB: imageUrls, aF: name, aQ: subBreeds};
	});
...
var $elm$http$Http$request = function (r) {
	return $elm$http$Http$command(
		$elm$http$Http$Request(
			{as: false, au: r.au, O: r.O, Q: r.Q, aE: r.aE, aS: r.aS, ak: r.ak, al: r.al}));
};
var $elm$http$Http$get = function (r) {
	return $elm$http$Http$request(
		{au: $elm$http$Http$emptyBody, O: r.O, Q: _List_Nil, aE: 'GET', aS: $elm$core$Maybe$Nothing, ak: $elm$core$Maybe$Nothing, al: r.al});
};
var $author$project$HttpClient$getBreeds = $elm$http$Http$get(
	{
		O: A2($elm$http$Http$expectJson, $author$project$Common$GotBreeds, $author$project$HttpClient$breedsDecoder),
		al: $author$project$HttpClient$fetchAllBreedsUrl
	});
var $author$project$Model$Loading = {$: 1};
var $author$project$Model$Model = F4(
	function (state, allBreeds, detailBreed, pageStartIdx) {
		return {ar: allBreeds, aw: detailBreed, aJ: pageStartIdx, aN: state};
	});
...
var $author$project$Model$Success = {$: 2};
...
var $author$project$HttpClient$getImageUrls = function (breedName) {
	return $elm$http$Http$get(
		{
			O: A2($elm$http$Http$expectJson, $author$project$Common$GotImageUrls, $author$project$HttpClient$imageUrlsDecoder),
			al: $author$project$HttpClient$fetchImageUrls(breedName)
		});
};
...
var $author$project$Update$updateAllBreeds = F2(
	function (newBreedMay, allBreeds) {
		if (newBreedMay.$ === 1) {
			...
			return A3($elm$core$Dict$insert, newBreed.aF, newBreed, allBreeds);
		}
	});
var $author$project$Update$update = F2(
	function (msg, model) {
		switch (msg.$) {
			case 2:
				var str = msg.a;
				var foundDetailBreedMay = A2($elm$core$Dict$get, str, model.ar);
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							aw: foundDetailBreedMay,
							aN: needToFetchImageUrls ? $author$project$Model$Loading : model.aN
						}),
					needToFetchImageUrls ? A3(
						$elm_community$maybe_extra$Maybe$Extra$unwrap,
						$elm$core$Platform$Cmd$none,
						function (breed) {
							return $author$project$HttpClient$getImageUrls(breed.aF);
						},
			...
			case 3:
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{aw: $elm$core$Maybe$Nothing, aJ: 0}),
					$elm$core$Platform$Cmd$none);
			case 4:
				var result = msg.a;
				if (!result.$) {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								ar: A2($author$project$Update$updateAllBreeds, newBreedMay, model.ar),
								aw: newBreedMay,
								aN: $author$project$Model$Success
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					return _Utils_Tuple2(
			...
			case 5:
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{aJ: model.aJ + $author$project$Common$numImagesPerPage}),
					$elm$core$Platform$Cmd$none);
			default:
				return _Utils_Tuple2(
					_Utils_update(
...
var $author$project$Common$Reload = {$: 0};
...
var $author$project$View$viewBreeds = function (model) {
	var subBreedsText = function (breed) {
		return $elm$core$List$isEmpty(breed.aQ) ? '' : $elm$core$String$concat(
			A2($elm$core$List$intersperse, ', ', breed.aQ));
	};
	var sortedBreeds = A2(
				...
				[
					...
					_List_fromArray(
						[
							...
							$elm$html$Html$Attributes$value(breed.aF),
							A2(
							$elm$html$Html$Events$on,
							'click',
...
var $author$project$Common$ReturnToBreedsList = {$: 3};
...
var $author$project$Common$NextPage = {$: 5};
var $author$project$Common$PrevPage = {$: 6};
...
var $author$project$View$pagingButtons = F3(
	function (firstIdx, lastIdx, numImages) {
		...
		return A2(
			$elm$html$Html$div,
			_List_Nil,
			shouldDisplayPaging ? _List_fromArray(
				[
					A2(
	...
	var numImages = $elm$core$Array$length(
		A2(
			...
			A2(
				...
				model.aw)));
	var lastIdx = A2($elm$core$Basics$min, numImages, model.aJ + $author$project$Common$numImagesPerPage);
	var firstIdx = model.aJ + 1;
	return A2(
		$elm$html$Html$div,
		_List_Nil,
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$text('<<< Return to Dog Breeds List')
					])),
				...
				_List_fromArray(
					[
						$elm$html$Html$text(
						'Displaying ' + ($elm$core$String$fromInt(firstIdx) + ('-' + ($elm$core$String$fromInt(lastIdx) + (' of ' + ($elm$core$String$fromInt(numImages) + ' images'))))))
					])),
				A2($elm$html$Html$br, _List_Nil, _List_Nil),
				A2($elm$html$Html$br, _List_Nil, _List_Nil),
		...
		_List_fromArray(
			[
				...
				switch (_v0.$) {
					...
					default:
						return $elm_community$maybe_extra$Maybe$Extra$isJust(model.aw) ? $author$project$View$viewDetails(model) : $author$project$View$viewBreeds(model);
				}
			}()
			]));
};
var $author$project$Main$main = $elm$browser$Browser$element(
	{
		aD: $author$project$Model$init,
		aR: function (_v0) {
			return $elm$core$Platform$Sub$none;
		},
		aU: $author$project$Update$update,
		aV: $author$project$View$view
	});
